    ...
    def calculateShortestPath(self, vertexList, edgeList, startVertex):
        ...
        for edge in edgeList:
            if self.hasCycle(edge):
                ...
                BellmanFord.HASCYCLE = True
                return

    def hasCycle(self, edge):
        if (edge.statVertex.minDistance + edge.weight) < edge.statVertex.minDistance:
            return True
        return False

    def getShortestPathTo(self, targetVertex):
        if not BellmanFord.HASCYCLE:
...
cycle = BellmanFord()
cycle.calculateShortestPath(vertexList,edgeList,node1)
cycle.getShortestPathTo(node7)
